...
var _ = Describe("Observability E2E Test Suite", func() {
	...
	It("Test: Verify creation of Rancher cluster output and cluster flow", Label("LEVEL1", "Logging", "E2E"), func() {
		...
		if deployLoggingResourcesError != nil {
			e2e.Logf("Failed to deploy cluster output and flow: %v", deployLoggingResourcesError)
		} else {
			...
			if strings.Contains(pod, "syslog-ng-deployment") {
				...
				if len(podFields) > 0 {
					...
					for attempt < maxRetries {
						...
						if strings.Contains(syslogLogsOutput, "cattle-logging-system") {
							e2e.Logf("Logs contain 'cattle-logging-system'. Test passed. %v", syslogLogsOutput)
						} else {
							e2e.Logf("Logs do not contain 'cattle-logging-system' Retrying... (Attempt %d/%d)\n", attempt+1, maxRetries)
							attempt++
