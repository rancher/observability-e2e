...
type InplaceParams struct {
	...
	ProvisioningInput   charts.ProvisiningConfig
}
...
var _ = DescribeTable("Test: Rancher inplace backup and restore test.",
	func(params InplaceParams) {
		...
		fmt.Println(userList, projList, roleList)
		Expect(err).NotTo(HaveOccurred())
		...
		// Ensure chart uninstall runs at the end of the test0
		DeferCleanup(func() {
		...
		s3_location := BackupRestoreConfig.S3BucketName + "/" + BackupRestoreConfig.S3FolderName
		result, err := s3Client.FileExistsInBucket(s3_location, filename)
		Expect(err).NotTo(HaveOccurred())
		...
		restoreTemplate := bv1.NewRestore("", "", charts.SetRestoreObject(params.BackupOptions.Name, BackupRestoreConfig.Prune))
	...
	// **Test Case: Ranacher inplace backup and restore test scenarios
