...
var _ = DescribeTable("Test: Validate the Backup and Restore Migration Scenario from RKE2 to RKE2",
	func(params MigrationParams) {
		...
		output, err := tfCtx.DestroyTarget("module.ec2.aws_instance.rke2_node")
		if err != nil {
			fmt.Println("Remove rke2_node destroy failed:", err)
		} else {
			fmt.Println("Destroy output:", output)
		}
		...
		// Now we need to start with all steps for the restore the backup
		// Hostname of the machine would be the same
		fmt.Print(userListPostBackup, projListPostBackup, roleListPostBackup)
		...
		chartDir, err := charts.DownloadAndExtractRancherCharts("dev-v2.11")
		Expect(err).NotTo(HaveOccurred(), "Failed to download and extract repo")
		fmt.Printf("Extracted charts directory: %s\n", chartDir)

		backupRestoreChartVersion := os.Getenv("BACKUP_RESTORE_CHART_VERSION")
		// install the rancher-backup-crd
		...
		// create the restore-migation yaml and apply it
		...
		// install the rancher using helm you are pretty done I believe
