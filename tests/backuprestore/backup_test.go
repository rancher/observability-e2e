...
var _ = DescribeTable("BackupTests: ",
	func(params BackupParams) {
		...
		fmt.Println("Waiting for 5 minutes to see backups appear...")
		time.Sleep(duration)
	...
	Entry("Test Rancher Backup retention with scheduled functionality", Label("LEVEL1", "only_backup", "s3", "2.8", "2.9", "2.10"), BackupParams{
		StorageType: "s3",
		BackupOptions: charts.BackupOptions{
	...
	Entry("Test Rancher Backup retention with scheduled functionality", Label("LEVEL1", "only_backup", "s3", "2.11"), BackupParams{
		StorageType: "s3",
		BackupOptions: charts.BackupOptions{
			Name:            namegen.AppendRandomString("backup"),
